...
# Source function
f = Expression('A * (exp(-alpha * pow(x[0] - x1, 2) - alpha * pow(x[1] - y1'
               ', 2)) + exp(-alpha * pow(x[0] - x2, 2) - alpha * pow(x[1] - '
               'y2, 2)) + exp(-alpha * pow(x[0] - x3, 2) - alpha * pow(x[1]'
               '- y3, 2)))', degree=2, A=A, alpha=alpha, x1=x1, x2=x2, x3=x3,
               y1=y1, y2=y2, y3=y3)

# Velocity vector field
velocity = Expression(('B * cos(omega * t)', 'B * sin(omega * t)'), degree=2,
                      B=B, omega=omega, t=0.0)
w = Function(W)
